...
<h2>講師個人情報一覧表</h2>
<input type="button" onclick="location.href='./fukugo.php'" value="講師検索">
<input type="button" onclick="location.href='./teacher1.php'" value="新規登録"><br><br>
<table>
...
foreach ($teacher_list as $teacher) {
    ...
    echo '<td>';
    echo '<a href="teacher4.php?id=',$teacher['teacherid'],'">編集</a>';
    echo '</td>';
    echo '</tr>';
    echo "\n";
}
foreach($pdo->query('select * from teacher where delete_flg is null') as $teacher) {
$teacher_obj  = $pdo->query('select * from teacher');
$sex_obj      = $pdo->query('select * from sex');
$domain_obj   = $pdo->query('select * from subject');
